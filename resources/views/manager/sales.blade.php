...
<div class="card">
    <div class="card-header">
        <form method="GET" style="display:flex;gap:8px;flex-wrap:wrap;margin:0">
            <select name="cashier" class="form-control" style="width:auto">
                <option value="">All Cashiers</option>
                @foreach($cashiers as $c)
                <option value="{{ $c->id }}" {{ request('cashier')==$c->id?'selected':'' }}>{{ $c->name }}</option>
                @endforeach
            </select>
            ...
            <input type="date" name="date_from" value="{{ request('date_from') }}" class="form-control" style="width:auto">
            <input type="date" name="date_to"   value="{{ request('date_to') }}"   class="form-control" style="width:auto">
    ...
    <div class="table-wrap">
        <table>
            <thead><tr><th>Receipt #</th><th>Customer</th><th>Cashier</th><th>Items</th><th>Total</th><th>Method</th><th>Status</th><th>Date</th></tr></thead>
            <tbody>
            ...
            <tr>
                ...
                <td style="color:var(--text-muted);font-size:.78rem">{{ $tx->created_at->format('M d, H:i') }}</td>
            </tr>
            @empty
            <tr><td colspan="8"><div class="empty-state"><i data-lucide="receipt"></i><p>No transactions found.</p></div></td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
